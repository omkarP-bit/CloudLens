...
function mockResources(accountId: string, filterRegion?: string, filterType?: string, filterState?: string): Resource[] {
  ...
  return mockData.filter((r) => {
    ...
    if (filterState && r.state !== filterState && r.state.toLowerCase() !== filterState.toLowerCase()) return false;
    return true;
  });
}

function buildRegionalClient<T>(creds: AWSCredentials, region: string, Client: new (config: any) => T): T {
  return new Client({
    region,
    credentials: {
...
export async function discoverResources(
  ...
  accountId: string,
  options: { region?: string; type?: ResourceType; state?: string } = {}
): Promise<Resource[]> {
  if (isMock(creds)) {
  ...
  return results.filter((r) => {
    if (options.state) {
      ...
      if (rState !== matchState) return false;
    }
    return true;
  });
}

async function discoverEC2(creds: AWSCredentials, accountId: string, region: string): Promise<Resource[]> {
  ...
  do {
    ...
    for (const reservation of response.Reservations || []) {
      for (const instance of reservation.Instances || []) {
        ...
        (instance.Tags || []).forEach((t) => { if (t.Key && t.Value) tags[t.Key] = t.Value; });
...
async function discoverRDS(creds: AWSCredentials, accountId: string, region: string): Promise<Resource[]> {
  ...
  do {
    ...
    for (const db of response.DBInstances || []) {
      const tags: Record<string, string> = {};
      (db.TagList || []).forEach((t) => { if (t.Key && t.Value) tags[t.Key] = t.Value; });
...
async function discoverECS(creds: AWSCredentials, accountId: string, region: string): Promise<Resource[]> {
  const client = buildRegionalClient(creds, region, ECSClient);
  const resources: Resource[] = [];

  const clusterArns = await client.send(new ListClustersCommand({}));
  if (!clusterArns.clusterArns?.length) return resources;
  ...
  for (const cluster of clusters.clusters || []) {
    ...
    if (serviceNames.length > 0) {
      ...
      for (const svc of services.services || []) {
        const tags: Record<string, string> = {};
        (svc.tags || []).forEach((t) => { if (t.key && t.value) tags[t.key] = t.value; });
...
async function discoverLambda(creds: AWSCredentials, accountId: string, region: string): Promise<Resource[]> {
...
async function discoverS3(creds: AWSCredentials, accountId: string): Promise<Resource[]> {
...
async function discoverElastiCache(creds: AWSCredentials, accountId: string, region: string): Promise<Resource[]> {
    ...
    for (const cluster of response.CacheClusters || []) {
      ...
      resources.push({
        ...
        metadata: {
          ...
          multiAZ: (cluster as any).MultiAZEnabled,
        },
        estimatedMonthlyCost: null,
        tags,
