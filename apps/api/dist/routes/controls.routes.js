...
import { executeAction } from '../services/aws/controls.service.js';
...
const validResourceTypes = ['EC2', 'RDS', 'ECS', 'Lambda'];
export async function controlsRoutes(fastify) {
    ...
    fastify.post('/api/controls/:resourceType/:resourceId/action', async (request, reply) => {
        const user = request.user;
        ...
        try {
            ...
            const result = await executeAction(creds, resourceType, resourceId, body.action, body.region);
            await insertAuditLog(supabaseAdmin, {
                user_id: user.id,
                account_id: body.accountId,
                action: `${resourceType}:${body.action}`,
