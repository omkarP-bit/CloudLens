...
import { sanitizeCommand, createSession, getSession, listSessions, closeSession, executeCommand, cleanExpiredSessions, clearSessions, } from './terminal.service.js';
...
async function mockSupabaseOnce(data) {
    const mod = await import('../../config/supabase.js');
    mod.supabaseAdmin.from = vi.fn(() => ({
        select: vi.fn(() => ({
            eq: vi.fn(() => ({
                eq: vi.fn(() => ({
