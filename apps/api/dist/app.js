...
import { saveAccount, listAccounts, deleteAccount, getDecryptedCredentials, updateAccountStatus, rotateCredentials, } from './services/supabase/accounts.repo.js';
...
fastify.post('/api/accounts', async (request, reply) => {
    ...
    try {
        ...
        const account = await saveAccount(user.id, body.alias, body.awsAccountId, body.roleArn, body.credentialType, body.regions, {
            accessKeyId: body.accessKeyId,
            secretAccessKey: body.secretAccessKey,
            sessionToken: body.sessionToken,
        });
        const { encrypted_access_key_id, encrypted_secret_access_key, encrypted_session_token, encrypted_dek, ...safeAccount } = account;
        return safeAccount;
    }
    catch (err) {
...
fastify.delete('/api/accounts/:id', async (request, reply) => {
    ...
    try {
        ...
        const { encrypted_access_key_id, encrypted_secret_access_key, encrypted_session_token, encrypted_dek, ...safeAccount } = deleted;
        return safeAccount;
    }
    catch (err) {
        fastify.log.error(err);
...
fastify.post('/api/accounts/:id/validate', async (request, reply) => {
    ...
    try {
        ...
        const { encrypted_access_key_id, encrypted_secret_access_key, encrypted_session_token, encrypted_dek, ...safeAccount } = updated;
