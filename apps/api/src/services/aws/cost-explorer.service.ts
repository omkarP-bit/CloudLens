...
function extractAmount(metrics: Record<string, { Amount?: string }> | undefined): number {
  if (!metrics) return 0;
...
async function fetchCostAndUsage(
  ...
  const amounts: { time: string; amount: number }[] = [];

  for (const period of resultsByTime) {
    const total = period.Groups
      ? period.Groups.reduce((sum, g) => {
          return sum + extractAmount(g.Metrics as any);
        }, 0)
      : extractAmount(period.Total as any);
...
async function fetchGroupedCost(
  creds: AWSCredentials,
  input: GetCostAndUsageCommandInput
) {
  if (creds.accessKeyId.startsWith('mock_') || creds.secretAccessKey.startsWith('mock_')) {
  ...
  const groups: { service: string; amount: number }[] = [];

  for (const period of response.ResultsByTime || []) {
    for (const group of period.Groups || []) {
      const service = group.Keys?.[0] || 'Unknown';
      const amount = extractAmount(group.Metrics as any);
