...
import { getCostSummary, getDailyTrends, getServiceBreakdown, getTopServices, getCredits, getDailyCredits, } from '../services/aws/cost-explorer.service.js';
import { z } from 'zod';
const CACHE_TTL = 60;
...
export async function costsRoutes(fastify) {
    fastify.get('/api/costs/summary', async (request, reply) => {
        const user = request.user;
        const { accountId } = z.object({ accountId: z.string().uuid() }).parse(request.query);
        try {
    ...
    fastify.get('/api/costs/trends', async (request, reply) => {
        const user = request.user;
    ...
    fastify.get('/api/costs/breakdown', async (request, reply) => {
        const user = request.user;
        const { accountId } = z.object({ accountId: z.string().uuid() }).parse(request.query);
        try {
            await verifyAccountOwnership(accountId, user.id);
    ...
    fastify.get('/api/costs/credits', async (request, reply) => {
        const user = request.user;
        const query = z.object({
            accountId: z.string().uuid(),
        ...
        if (query.days)
            cacheKeyParts.push(String(query.days));
    ...
    fastify.get('/api/costs/top-services', async (request, reply) => {
        const user = request.user;
        const query = z.object({
            accountId: z.string().uuid(),
