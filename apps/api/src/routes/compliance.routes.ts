...
import {
  ...
  listFindings,
  getComplianceStats,
} from '../services/supabase/compliance.repo.js';
...
export async function complianceRoutes(fastify: FastifyInstance) {
  fastify.get('/api/compliance/policies', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.get('/api/compliance/policies/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    const user = (request as any).user;
  ...
  fastify.post('/api/compliance/policies', async (request: FastifyRequest, reply: FastifyReply) => {
    const user = (request as any).user;
    ...
    try {
      const policy = await createPolicy(user.id, {
        ...
        rule_definition: body.ruleDefinition as any,
        action: body.action,
  ...
  fastify.patch('/api/compliance/policies/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    const user = (request as any).user;
    const { id } = request.params as { id: string };
    ...
    try {
      ...
      if (body.ruleDefinition) updateData.rule_definition = body.ruleDefinition;
  ...
  fastify.delete('/api/compliance/policies/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    const user = (request as any).user;
    const { id } = request.params as { id: string };
  ...
  fastify.get('/api/compliance/findings', async (request: FastifyRequest, reply: FastifyReply) => {
    const user = (request as any).user;
    const query = z.object({
      accountId: z.string().uuid().optional(),
  ...
  fastify.get('/api/compliance/stats', async (request: FastifyRequest, reply: FastifyReply) => {
    const user = (request as any).user;
    const query = z.object({
      accountId: z.string().uuid().optional(),
    }).parse(request.query as any);
