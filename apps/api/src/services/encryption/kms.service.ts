 ...
 */
export async function generateDEK(): Promise<{ dek: Buffer; encryptedDek: string }> {
  ...
  try {
    const { Plaintext, CiphertextBlob } = await kms.send(
      new GenerateDataKeyCommand({ KeyId: CMK_ARN, KeySpec: 'AES_256' })
    );
    return {
 ...
 */
export async function decryptDEK(encryptedDek: string): Promise<Buffer> {
